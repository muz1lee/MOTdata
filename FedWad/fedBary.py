...
class localOT:
    ...
    def fit(self, xs, xt, ws=None, wt=None, approx_interp=True,
        ...
        for i in range(self.n_localepoch):
            ...
            G, weight_G, cost_g, splan = interp_G.int_m, interp_G.weights, interp_G.cost, interp_G.plan
            interp_G.int_init = G
            # on client T
...
if __name__ == '__main__':
    ...
    mu_bary = np.array([0, 1])
    cov_bary = np.array([[1, 0], [0, 1]])
    ...
    random_val_init = 1
    num_clients= 3
    int_m = [np.random.randn(n_supp, dim) * random_val_init]* num_clients
    ...
    for i in range(n_epoch):
        ...
        for idx,client in enumerate(client_list):
            fedot = localOT(n_supp=100, n_localepoch=n_localepoch, t_val=t_val).fit(client, int_m[idx])
            cost_lc.append(fedot.cost)
            local_eta.append(fedot.int_meas)
        ...
        for idx in range(num_clients):
            fedot_server = localOT(n_supp=100, n_localepoch=n_localepoch, t_val=t_val).fit(x_bary, int_m[idx])
            gamma = localOT(n_supp=100, n_localepoch=n_localepoch, t_val=t_val).fit(fedot_server.int_meas, local_eta[idx])
            tplan = fedot_server.tplan
            plan = fedot_server.plan
            support = np.dot(fedot_server.baryplan,x_bary)
            cost_ls.append(fedot_server.cost)
            global_eta.append(fedot_server.int_meas)
            global_gamma.append(gamma.int_meas)

            supports.append( support)
            new_Q += np.array(support)
        x_bary = new_Q
        int_m = global_gamma
        total_cost = np.sum(cost_lc)+ np.sum(cost_ls)

...
import geomloss
from corrupt_data import load_data_corrupted
...
class FedOT:
    ...
    def fit(self, xs, xt, ws=None, wt=None, approx_interp=True,
        ...
        for i in range(self.n_epoch):
            ...
            # on server
            cost = cost_g + cost_h
            print('{}-th epoch the cost is {}'.format(i,cost))
...
def detections_case(training_size,valid_size,resize,portion, n_supp ,n_epoch ,t_val=0.5):
    ...
    fedot_pt1 = FedOT(n_supp=n_supp, n_epoch=n_epoch, metric='sqeuclidean', t_val=t_val,get_int_list=True)
    fedot_pt1 = fedot_pt1.fit(XA, XT.requires_grad_(True))
    int_meas1 = fedot_pt1.int_meas
    client_int_meas =fedot_pt1.list_int_G[-1]
    server_int_meas = fedot_pt1.list_int_H[-1]
    ...
    dual1 = np.array(loss(t_c_meas, t_s_meas)[0])
    dual2 = np.array(loss(t_XA, t_int_meas1)[0])
    dual3 = np.array(loss(t_XA, t_XT)[0])
    dual4 = np.array(loss(t_XA, t_s_meas)[0])
    dual5 = np.array(loss(t_c_meas, t_XT)[0])

    duals = [dual1,dual2,dual3,dual4,dual5]
    ls = []
    for dual in duals:
...
if __name__ == '__main__':
    ...
    if detection:
        ...
        n_supp = 1000
        n_epoch = 1
        for portion in portion_l:
            print('portion {}'.format(portion))
            ls = detections_case(training_size, valid_size, resize, portion,n_supp,n_epoch)
            result.append(ls)

    data = pd.DataFrame(result)
    data.columns= ['r1','c1','r2','c2','r3','c3','r4','c4','r5','c5']
    ...
    ax1.set_xlabel('Noise Portion', fontsize=28)

    color1 = '#FFCC99'
    color2 = '#99CCCC'
    color3 = '#99CC99'
    alpha = 0.8
    ax1.bar(data.index, data['c3'], width=width, alpha=alpha, color=color1, label='c-access data')
    ax1.bar(data.index - width, data['c2'], width=width, alpha=0.7, color=color2, label='c-gamma')
    ax1.bar(data.index + width, data['c4'], width=width, alpha=alpha, color=color3, label='c-eta')
    ...
    ax2 = ax1.twinx()
    ax2.set_xlabel('Noise Portion',fontsize=28)
    ax2.set_ylabel('Detect Ratio',fontsize=28)

    ax2.plot(data.index, data['r3'], color='#993333',linestyle= '--',linewidth=3, marker='*',markersize=14,label='r-access data')
    ax2.plot(data.index, data['r2'], color='#0099CC', linestyle='--', linewidth=3, marker='*', markersize=14,label='r-gamma')
    ax2.plot(data.index, data['r4'], color='green', linestyle= '--',linewidth=3, marker='*',markersize=14,label='r-eta')
    ax2.plot(data.index, data['r1'], color='#996699',linestyle= '--',linewidth=3, marker='*',markersize=14,label='r-servergamma')
    ax2.plot(data.index, data['r5'], color='gray', linestyle= '--',linewidth=3, marker='*',markersize=14,label='r-servereta')
    ...
    # ax1.legend(lines + bars, labels + bar_labels, loc='best',fontsize=22)

    # legend = plt.legend(lines + bars, labels + bar_labels, loc='best', fontsize=22)
    #

...
def efficient_pwdist_gauss(M1, S1, M2=None, S2=None, sqrtS1=None, sqrtS2=None,
    ...
    D = torch.zeros((n1, n2), device = 'cpu', dtype=M1.dtype)
...
def pwdist_exact(X1, Y1, X2=None, Y2=None, symmetric=False, loss='sinkhorn',
    ...
    if cost_function == 'euclidean':
        if p == 1:
            ...
            cost_function = lambda x, y: geomloss.utils.squared_distances(x, y) #[x] pwdist_exact 定义cost function.
        else:
            raise ValueError()

    if loss == 'sinkhorn':
        distance = geomloss.SamplesLoss( #[x] 定义sinkhorn distance函数
            loss=loss, p=p,
            cost=cost_function,
            debias=debias,
            blur=entreg**(1 / p),

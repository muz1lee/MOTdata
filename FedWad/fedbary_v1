...
if __name__ == '__main__':
    ...
    for i in range(n_epoch):
        ...
        for idx in range(num_users):
            ...
            # server计算三个不同的eta_Q
            fedot_server = localOT(n_supp=100, n_localepoch=n_localepoch, t_val=t_val,server= False).fit(x_bary, int_m[idx],wint_m =weight_int_m[idx],
                                                                                                        learn_support =False )
            ...
            #更新Q
            sourceplan = fedot_server.cost
            support = np.dot(sourceplan, int_m[idx])
            #更新gamma

...
class DatasetDistance():
    ...
    def __init__(self, D1=None, D2=None,
        ...
        self._pwlabel_stats_1 = None # D1的label间distance
        self._pwlabel_stats_2 = None # D2的label间distance
    ...
    def _init_data(self, D1, D2):
        ...
        if (targets1 is None) or self.ignore_source_labels:
            # 列举出targets1中元素的集合，并给出每个元素出现的次数。
        ...
        if (targets2 is None) or self.ignore_target_labels:
            # reindex_start是用于D2的类别indexes会基于D1的类别之后进行标记；若reindex_start=True，则表示不需要调整D2的标签。
        ...
        # if not torch.equal(classes1, self.V1): # 防止出现classes1有[1,5,9]三个类别，self.V1也应该是[1,5,9]，则会发现9这个超出索引了。
        #     self.classes1 = self.V1
        # if not torch.equal(classes2, self.V2): # 防止出现classes1有[1,5,9]三个类别，self.V1也应该是[1,5,9]，则会发现9这个超出索引了。
    ...
    def _get_label_stats(self, side='both'):
        ...
        if (side=='both' or side == 'src') and (self.Means[0] is None):
            ...
            M1, C1 = compute_label_stats(self.D1, self.targets1, self.idxs1,
                                         ...
                                         **shared_args)
            # For MNIST dataset, mean is (10,784) and covariance is (10,784,784)
    ...
    def _get_label_distances(self): #XXX 计算1次label distance，后面直接取用。
        ...
        if self.inner_ot_method == 'gaussian_approx':
            ...
            pwdist = partial(pwdist_exact, #[x] - 定义label distance函数
                             symmetric=self.symmetric_tasks,
                             p = self.inner_ot_p,
        ...
        if self.debiased_loss and not self.symmetric_tasks:
            ...
            if self._pwlabel_stats_1 is None:
                ...
                if self.inner_ot_method == 'gaussian_approx':
                    ...
                    DYY1 = pwdist(self.X1, self.Y1) #[x] - 计算X1和Y1的distance
            else:
                if self.inner_ot_method == 'gaussian_approx':
            ...
            if self._pwlabel_stats_2 is None:
                ...
                if self.inner_ot_method == 'gaussian_approx':
                    ...
                    DYY2 = pwdist(self.X2, self.Y2) #[x] - 计算X2和Y2的distance
            else:
        ...
        if self.inner_ot_method == 'gaussian_approx':
            ...
            DYY12 = pwdist(self.X1,self.Y1,self.X2, self.Y2) #[x] - 计算X1,Y1,X2,Y2的distance
            DYY12_means = None
        ...
        if self.debiased_loss:
            D = torch.cat([torch.cat([DYY1, DYY12], 1),
                           torch.cat([DYY12.t(), DYY2], 1)], 0) #[x] - 将三种distance矩阵拼接
            if self.inner_ot_method in ['gaussian_approx', 'naive_upperbound', 'means_only']:
        ...
        if self.inner_ot_method == 'gaussian_approx':
            ...
            self._pwlabel_stats_1 = {'dlabs':DYY1, 'dmeans':DYY1_means}#, 'sqrtΣ':sqrtΣ1}
            self._pwlabel_stats_2 = {'dlabs':DYY2, 'dmeans':DYY2_means}#, 'sqrtΣ':sqrtΣ2}
        else: #TODO - 存储D1和D2各自的label间距离
            self._pwlabel_stats_1 = {'dlabs':DYY1}#
            self._pwlabel_stats_2 = {'dlabs':DYY2}#, 'dmeans':DYY2_means, 'sqrtΣ':sqrtΣ2}


        return self.label_distances #[x] - 返回拼接好的distance矩阵

    def augmentation_data(self, maxsamples=10000, return_coupling=False):  # XXX distance函数
        ...
        if self.method == 'augmentation':
            ...
            if not hasattr(self, 'XμΣ1') or self.XμΣ1 is None:
                XA = augmented_dataset(DA, self.Means[0], self.Covs[0],
                                       maxn=maxsamples)  # diagonal_cov=self.diagonal_cov)
                del DA
                XB = augmented_dataset(DB, self.Means[1], self.Covs[1],
                                       maxn=maxsamples)  # diagonal_cov=self.diagonal_cov)
                del DB
                self.XμΣ1 = XA
                self.XμΣ2 = XB
            ...
            # 这里我把计算distance给删掉了，只留下augmentated data
        ...
        #     return dist, π
        # else:
        #     return dist  # XXX 返回计算好的OT距离

    def distance(self, maxsamples=10000, return_coupling=False):#XXX distance函数
        """ Compute dataset distance.
        ...
        if self.X1 is None or self.X2 is None: 
            if (not self.method == 'jdot') and (self.λ_y is not None) and (self.λ_y > 0):
                s = time()
                _ = self._get_label_distances() #[x] - 获得label distance
                logger.info('/* Time to precompute label distances: {} */'.format(time() - s))

            self._load_datasets(maxsamples, device=device_dists) #[x] - 加载数据集
        if self.method == 'augmentation':
            DA = (self.X1, self.Y1)
            DB = (self.X2, self.Y2)
            ...
            if self.λ_y is None or self.λ_y == 0:
                ...
                W = self._get_label_distances().to(torch.device(device_dists)) #[x] - 通过label distance得到cost matrix. size=(C1+C2,C1+C2)
            ...
            cost_geomloss = partial(
                ...
                λ_x=self.λ_x, # 1.0
                λ_y=self.λ_y, # 1.0
                feature_cost=self.feature_cost # 'euclidean'
            )

            loss = geomloss.SamplesLoss(
                loss=self.loss, p=self.p, # 'sinkhorn', 2
                cost=cost_geomloss,
                debias=self.debiased_loss, # True
                blur=self.entreg**(1 / self.p), # 0.1, 2
                backend='tensorized'
            )
    ...
    def subgroup_distance(self, labels_a, labels_b, maxsamples=500):
        ...
        if self.method == 'augmentation':
            ...
            #FIXME 似乎cost_geomloss没有被调用，这里partial传入的参数实际上是有问题的。但这个函数并没有被调用。
            ## This one leverages precomputed pairwise label distances
...
def batch_augmented_cost(Z1, Z2, W=None, Means=None, Covs=None, feature_cost=None,
                         p=2, λ_x=1.0, λ_y=1.0): #XXX batch_augmented_cost函数（总的cost matrix）
    ...
    if λ_y is None or λ_y == 0:
        ...
        #[x] - label-label distance提前计算好了，直接取用。
        M = W.shape[1] * Y1[:, :, None] + Y2[:, None, :] # tricky，取出对应的label-label对。
    ...
    D = λ_x * C1  +  λ_y * (C2/p) #[x] - 总的cost matrix.
